...
# WebSocket Channel Protocol v1.1
...
```
GET /v1/ws?session_id=xxx HTTP/1.1
...
```
ws://{host}:{port}/v1/ws?session_id={sid}&user_id={uid}
```
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `session_id` | 否 | 会话 ID。不传则服务端生成 UUID |
| `user_id` | 否 | 用户标识，用于权限校验和审计 |

URL path 中的 `v1` 为协议主版本号，不同主版本为不兼容协议。

### 3.2 Handshake

WebSocket 连接建立后，服务端立即推送 `session.created` 事件（首条消息）。客户端 **MUST** 等待收到此事件后再发送消息。
...
| `session.capabilities.client_tools` | bool | 是否启用客户端工具执行 |
...
3. 重连时在 URL 中传入原 `session_id` 以恢复会话上下文
4. 重连成功后收到新的 `session.created`
...
```bash
npm i -g wscat
wscat -c "ws://localhost:8081/v1/ws?session_id=test1"
...
async def main():
    uri = "ws://localhost:8081/v1/ws?session_id=test1"
...
ws, _, _ := websocket.Dial(ctx, "ws://localhost:8081/v1/ws?session_id=test1", nil)
