// Package websearch implements the web_search tool using the iFlytek
...
func (t *WebSearchTool) Execute(ctx context.Context, input json.RawMessage) (*types.ToolResult, error) {
	...
	// follows up with web_fetch (see two-stage retrieval design below).
	body := map[string]any{
	...
	//                          web_fetch on that URL to get the full page
	//
	// Why summary-only:
...
- 第 2 段（web_fetch）：摘要不够回答时，对挑中的 URL 调 web_fetch 拿全文。

使用规范：
- 传入一个搜索 query 找相关网页。
- 最多返回 N 条结果，每条含 Title / URL / 简短 Summary（通常 100~300 字）。
- 先快速扫摘要——对"是什么 / 谁 / 何时"这类事实型问题，摘要往往已经够答。
- 需要细节（正文全文、代码片段、原句）时，挑最相关的 URL 调 web_fetch。
- 不要对每个 URL 都 web_fetch——只在摘要确实不够时才取全文。

为什么先看摘要：
- 5 条结果的正文加起来 15KB 起步，会强制截断上下文。
- 摘要让视野更宽（看到全部结果），只对值得的 URL 付全文成本。

需要联网查信息时，先调本工具，再视情况 web_fetch。不要猜 URL——先搜索。`
