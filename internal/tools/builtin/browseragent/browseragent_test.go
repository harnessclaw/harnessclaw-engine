...
func TestBrowserAgentTool_LongRunningAndSafety(t *testing.T) {
	tl := New(agentrun.New(&fakeSpawner{}), config.BrowserAgentConfig{Enabled: true, MaxSteps: 30}, zap.NewNop())
...
func TestBrowserAgentTool_RejectsBlockedStartURL(t *testing.T) {
	tl := New(agentrun.New(&fakeSpawner{}), config.BrowserAgentConfig{
		Enabled:        true,
		MaxSteps:       30,
...
func TestBrowserAgentTool_SpawnsBrowserSubAgent(t *testing.T) {
	spawner := &fakeSpawner{}
	tl := New(agentrun.New(spawner), config.BrowserAgentConfig{Enabled: true, MaxSteps: 30}, zap.NewNop())
