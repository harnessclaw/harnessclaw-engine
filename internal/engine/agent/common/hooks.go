...
//   - submit not present + other tool calls → continue
...
// to 1 so callers cannot accidentally disable enforcement.
func ContractEnforcer(expected []types.ExpectedOutput, maxRetries int) loop.TurnHook {
	if maxRetries < 1 {
		maxRetries = 1
	}
	failures := 0
