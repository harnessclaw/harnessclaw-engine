...
func RenderSubAgentContract(def *AgentDefinition) string {
	...
	b.WriteString("2. **\"做不到\"的合法表达**：任务确实做不了时（缺输入、超能力、约束矛盾），调 meta_write({status: \"failed\", summary: \"做不到的原因 + 建议下一步\"}) 然后 submit_task_result({}) 退出 —— 不要硬写一个差产物。这条退出路径不算失败，是诚实退出。\n\n")
	...
	if len(def.Limitations) > 0 {
		b.WriteString("不做事项（命中即用 meta_write(status=\"failed\") + submit_task_result 诚实退出）：\n")
