...
// Deps is the dependency surface plan_agent needs from the host engine.
//
...
// Module is the plan_agent tier runtime.
type Module struct {
	deps Deps
}

// New constructs a plan_agent Module.
func New(deps Deps) *Module {
	return &Module{deps: deps}
}

// SubagentType returns "plan_agent" — the typed key the Spawner
// resolves to route plan-mode spawns at this module.
func (m *Module) SubagentType() string { return "plan_agent" }

// Run executes the plan_agent L3 LLM loop: build sub-session, render
// PlanAgentProfile system prompt, build tool pool (dispatch tools
// stripped so the planner cannot recursively spawn), emit
// subagent.start, run loop with StopOnSubmitResult, emit subagent.end,
// return SpawnResult.
func (m *Module) Run(ctx context.Context, cfg *agent.SpawnConfig) (*agent.SpawnResult, error) {
	...
	// plan_agent is L3: strip dispatch tools so it cannot recursively
	...
	sysPrompt := common.BuildSubAgentPrompt(common.PromptArgs{
		Ctx:               ctx,
		Session:           sess,
		Profile:           prompt.PlanAgentProfile,
	...
	if maxTurns <= 0 {
		// plan_agent typically needs more turns than a generic worker
	...
	loopRes, err := loop.Run(ctx, &loop.Config{
		...
		AgentScope:     common.BuildAgentScope(cfg, m.deps.RootDir, "plan_agent"),
