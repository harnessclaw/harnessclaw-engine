...
// Factory builds LeafContexts by wiring a real AgentSpawner into the
// LeafContext's SpawnFn. It replaces the legacy subagent.QueryEngineFactory;
// the package-global SetOutCh / currentOutCh holders are gone — the
// per-Run event channel is supplied by the EventRegistry, keyed on the
// root task ID submitted by agentrun.ModeScheduled.
type Factory struct {
	...
	events        *EventRegistry       // optional
	reader        tstate.Reader        // required when events is non-nil
}
...
func NewFactory(spawner agent.AgentSpawner, rootDir, rootSessionID string) *Factory {
	return &Factory{
		spawner:       spawner,
		rootDir:       rootDir,
...
func (f *Factory) WithStagingAndBus(staging tstate.StagingWriter, bus msgbus.Bus) *Factory {
	f.staging = staging
	f.bus = bus
	return f
}
...
// Returns the same factory for fluent chaining.
func (f *Factory) WithAgentResolver(r router.AgentResolver) *Factory {
	f.agentResolver = r
	return f
}

// WithEvents attaches a per-task event channel registry and the tstate
// reader used to walk the parent chain when looking up the channel for
// a given leaf task. Both arguments are required together — passing one
// without the other is a no-op (the factory degrades to "events drop").
// Returns the same factory for fluent chaining.
func (f *Factory) WithEvents(events *EventRegistry, reader tstate.Reader) *Factory {
	f.events = events
	f.reader = reader
	return f
}
...
func (f *Factory) Build(taskID types.TaskID, sessionID string, sp spec.TaskSpec) LeafContext {
	...
	// Per-task event channel: walk the parent chain to find the
	// registered outCh for the original Run that submitted this task's
	// ancestor. Nil-tolerant — events drop silently when no registry
	// or reader is configured.
	if f.events != nil && f.reader != nil {
		cfg.ParentOut = f.events.Find(context.Background(), f.reader, taskID)
	}
