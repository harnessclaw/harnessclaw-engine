...
func (f *stubFactory) Build(taskID types.TaskID, sessionID string, sp spec.TaskSpec) worker.LeafContext {
	sid := sessionID
	if sid == "" {
		sid = string(taskID)
	}
...
func (s *stubWS) WriteMeta(ctx context.Context, m workspace.Meta) (string, error) {
	return worker.WriteMeta(ctx, s.root, s.root, m)
}
...
func newTestEnv(t *testing.T) (*scheduler.Scheduler, tstate.Kernel, msgbus.Bus, *worker.ConsumerPool, func()) {
	t.Helper()
	tempRoot := t.TempDir()
	...
	pool := worker.NewConsumerPool(bus, kernel, factory, 2)
	pool.Start(ctx)
