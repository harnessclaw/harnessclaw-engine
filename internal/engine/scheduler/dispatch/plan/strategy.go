...
// Phase 1: plan_agent writes task breakdown to plan.json.
// Phase 2: plan_executor_agent dispatches tasks via freelance and updates plan.json.
...
//  1. plan_agent leaf → writes task breakdown to plan.json
//  2. plan_executor_agent leaf → reads plan.json, dispatches tasks, updates status
func (p *Strategy) Run(ctx context.Context, taskID types.TaskID, deps dispatch.Deps) (types.MetaRef, error) {
	...
	// ── Phase 1: plan_agent writes task breakdown ────────────────────────────
	...
	// Guard: verify plan.json has tasks (plan_agent may have failed silently).
	...
	// ── Phase 2: plan_executor_agent executes the plan ───────────────────────
	...
	if execRes.Status != "done" {
		return "", &dispatch.LeafFailedError{Reason: "plan_executor_agent: " + execRes.Reason}
	}
...
func (p *Strategy) requireNonEmptyPlan(sessionID string) error {
	...
	if len(plan.Tasks) == 0 {
		return fmt.Errorf("plan_agent wrote no tasks")
	}
	return nil
}
