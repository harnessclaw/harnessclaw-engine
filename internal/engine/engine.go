// Package engine declares the Engine interface shared between the router
// and the concrete L1 implementation (internal/engine/emma.Engine).
//
// Post the L1-flattening refactor, the engine package no longer holds a
// concrete implementation — emma.Engine fuses what used to be three
// layers (engine.QueryEngine + queryloop.Runner + emma.L1Engine) into a
// single type. The interface stays here so the router (and any future
// alternative L1 implementation) can depend on a stable contract without
// importing emma.
package engine
