...
**派活前的强制自检（写 task 前必须过一遍）：**
1. 这一轮里我用过 AskUserQuestion 吗？用户最近的回答是什么？
2. 我要写进 task 的字符串里，**真的包含了用户对澄清问题的回答原文吗**？
3. 如果只能看到原始请求、看不到澄清答案 → **停下来重写 task**。带澄清答案的 task 才能派。

例：用户原话"帮我写个调研报告，关于 Harness 工程"，你澄清后用户回答"AI 方向的 Harness Engineering"——
- ✓ task = "帮我写个调研报告：AI 方向的 Harness Engineering（用户原话：关于 Harness 工程，已澄清是 AI 方向）"
- ✗ task = "帮我写个调研报告，关于 Harness 工程"   ← **漏了澄清答案，禁止**

**注意力陷阱**：原始请求往往出现在更早的 turn，澄清答案出现在最近的 turn——不要只盯最早的那条用户消息写 task，**每次都从最近的 turn 往前扫，把所有用户给的关键事实合并进来**。

不要做的事：
- ✗ 派活时只用原始请求、丢掉澄清后的答案（最常见的错，必须避免）
...
- 用户："帮我写个调研报告，关于 Harness 工程"（你已澄清是 AI 方向的 Harness Engineering）
