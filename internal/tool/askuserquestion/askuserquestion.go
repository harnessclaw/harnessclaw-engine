// Package askuserquestion implements the ask_user_question tool — emma's
...
//
// Execution model: ask_user_question is a *client-routed* tool. Under the
...
// Tool is the ask_user_question tool implementation.
type Tool struct {
	tool.BaseTool
	logger *zap.Logger
}

// New constructs an ask_user_question tool. logger may be nil.
...
func (t *Tool) ValidateInput(raw json.RawMessage) error {
	...
	if err := json.Unmarshal(raw, &in); err != nil {
		return fmt.Errorf("invalid ask_user_question input: %w", err)
	}
...
func (t *Tool) Execute(_ context.Context, raw json.RawMessage) (*types.ToolResult, error) {
	...
	t.logger.Warn("ask_user_question.Execute reached server-side; this tool requires client_tools=true to function")
	return &types.ToolResult{
		Content: "ask_user_question is a client-routed tool — the running engine does not have a connected WebSocket client to deliver the question to. Configure client_tools=true.",
		IsError: true,
	}, nil
}
...
不要用 ask_user_question：
- 答案在对话历史里——回去重读。
- 有合理默认值——按默认走，并告诉用户你假设了什么。
- 一搜即得（改用 web_search / tavily_search）。
...
**收到回答后的强制提醒**：你接下来如果要派 scheduler 或调用任何工具，**必须把用户的澄清答案显式拼进 task / prompt 字段**——不要只用原始请求、丢掉澄清答案。原始请求往往在更早的 turn，澄清答案在最近的 turn，写 task 前两段都要扫一遍合并起来。`
