...
func TestQueryEngineFactory_Build_SetsSpawnFn(t *testing.T) {
	dir := t.TempDir()
	spawner := &fakeSpawner{}
	factory := subagent.NewQueryEngineFactory(spawner, dir, "root-sess")
...
func TestQueryEngineFactory_Build_SpawnFnCallsSpawnSync(t *testing.T) {
	dir := t.TempDir()
	spawner := &fakeSpawner{}
	factory := subagent.NewQueryEngineFactory(spawner, dir, "sess-1")
	sp := spec.TaskSpec{
